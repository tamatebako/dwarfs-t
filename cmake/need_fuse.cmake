    ...
    if(USE_FUSE_T)
      ...
      if(FUSE_T_FOUND)
        ...
        if(NOT TARGET PkgConfig::FUSE)
          ...
          # Fix invalid include directories in the imported target
          # The FUSE-T pkg-config may point to /usr/local/include/fuse which doesn't exist
          get_target_property(_fuse_inc_dirs PkgConfig::FUSE INTERFACE_INCLUDE_DIRECTORIES)
          if(_fuse_inc_dirs)
            set(_clean_inc_dirs "")
            foreach(_dir IN LISTS _fuse_inc_dirs)
              if(EXISTS "${_dir}")
                list(APPEND _clean_inc_dirs "${_dir}")
              endif()
            endforeach()
            if(_clean_inc_dirs)
              set_target_properties(PkgConfig::FUSE PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${_clean_inc_dirs}")
            endif()
          endif()
        endif()
