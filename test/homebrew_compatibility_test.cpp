...
// Test 5: Verify string_table.buffer is correctly read from legacy archives
// This test specifically checks for the frozen2_deserializer bug where
// buffer was incorrectly read as optional when it's not.
// See: docs/issues/frozen2_string_table_issue.md
TEST_F(LegacyThriftCompatibilityTest, StringTableBufferReadsCorrectly) {
  // This test requires the v0.14.1 compat image (uses compact_names/string_table)
  // If the file is not available, skip the test - do NOT fall back to other images
  // as they may not have the string_table feature being tested.
  fs::path image_file = fs::path(TEST_DATA_DIR) / "compat" / "fixtures" / "compat-v0.14.1.dwarfs";

  if (!fs::exists(image_file)) {
    GTEST_SKIP() << "compat-v0.14.1.dwarfs not available (test/compat/fixtures/ is gitignored)";
  }

  std::cout << "\n=== String Table Buffer Test: " << image_file.filename() << " ===" << std::endl;
  ...
  ASSERT_NO_THROW({
    ...
    // Walk filesystem and collect filenames
    ...
    std::cout << "Found " << filenames.size() << " entries:" << std::endl;
    for (const auto& name : filenames) {
      std::cout << "  - " << name << std::endl;
    }

    // Critical check: filenames should NOT be placeholders like "file_1", "file_2", etc.
    // If string_table.buffer was incorrectly read, names would be empty or placeholders.
    bool has_placeholder_names = false;
    for (const auto& name : filenames) {
      // Check for placeholder pattern: "file_N" where N is a number
      if (name.rfind("file_", 0) == 0 && name.size() > 5) {
        std::string suffix = name.substr(5);
        if (std::all_of(suffix.begin(), suffix.end(), ::isdigit)) {
          has_placeholder_names = true;
          std::cout << "WARNING: Found placeholder name: " << name << std::endl;
        }
      }
    }

    // If we have entries, they should have real names (not placeholders)
    if (!filenames.empty()) {
      EXPECT_FALSE(has_placeholder_names)
          << "Filenames should be real names, not placeholders. "
          << "This indicates string_table.buffer is being read correctly.";
    }

    // Should have at least some entries
    EXPECT_GT(filenames.size(), 0u) << "Expected to find filesystem entries";
  });
}
