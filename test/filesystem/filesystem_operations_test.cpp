...
// Test for highly compressible (sparse) large files
// This reproduces the SIGBUS crash that occurred on macOS ARM64
// when reading 10MB+ files that compress to very small sizes
TEST_F(FilesystemOperationsTest, highly_compressible_large_file) {
  // Create a 10MB file filled with zeros (highly compressible)
  // This compresses from 10MB to ~100 bytes with zstd level 9
  const size_t fileSize = 10 * 1024 * 1024;  // 10MB
  std::string content(fileSize, '\0');  // All zeros

  input_->add("", {1, 040755, 1, 1000, 100, 10, 42, 0, 0, 0});
  input_->add("10mb_zeros.bin", {2, 0100644, 1, 1000, 100,
                                 static_cast<uint32_t>(content.size()), 42, 0, 0, 0},
              content);
  ...
  auto file = filesystem_->find("/10mb_zeros.bin");
  ASSERT_TRUE(file);
  EXPECT_FALSE(file->inode().is_directory());

  auto st = filesystem_->getattr(file->inode());
  EXPECT_EQ(st.size(), fileSize);

  // Read the entire file in chunks to test the block cache
  // Use open() + read() pattern like verify_file() does
  int fh = filesystem_->open(file->inode());
  ASSERT_GE(fh, 0);
  ...
  while (total_read < fileSize) {
    size_t toRead = std::min(chunkSize, fileSize - total_read);
    size_t bytesRead = filesystem_->read(fh, buffer.data() + total_read,
                                         toRead, offset);
    EXPECT_EQ(bytesRead, toRead);
    total_read += bytesRead;
    offset += bytesRead;
  }

  EXPECT_EQ(total_read, fileSize);
  ...
  EXPECT_TRUE(allZeros) << "File content should be all zeros";
}

// Test reading from the large.dwarfs fixture file
// This tests the exact archive that triggered the SIGBUS crash on macOS ARM64
TEST_F(FilesystemOperationsTest, large_dwarfs_fixture) {
  // Load the fixture file content
  std::string fixturePath = TEST_DATA_DIR "/fixtures/large.dwarfs";
  std::ifstream fixtureFile(fixturePath, std::ios::binary);
  ASSERT_TRUE(fixtureFile.is_open()) << "Failed to open fixture: " << fixturePath;

  std::stringstream ss;
  ss << fixtureFile.rdbuf();
  std::string fsimage = ss.str();
  ASSERT_FALSE(fsimage.empty()) << "Fixture file is empty";

  // Create file_view and filesystem from the fixture
  file_view fileView = create_file_view(fsimage);
  ASSERT_TRUE(fileView.valid()) << "Failed to create file_view from fixture";

  reader::filesystem_options opts;
  opts.block_cache.max_bytes = 1 << 20;  // 1MB cache
  filesystem_ = std::make_unique<reader::filesystem_v2>(
      logger_, *input_, fileView, opts);

  // Find the 10mb.bin file
  auto file = filesystem_->find("/10mb.bin");
  ASSERT_TRUE(file) << "Could not find /10mb.bin in fixture";
  EXPECT_FALSE(file->inode().is_directory());

  // Get file size
  auto st = filesystem_->getattr(file->inode());
  const size_t fileSize = st.size();
  EXPECT_EQ(fileSize, 10 * 1024 * 1024);  // 10MB

  // Open and read the entire file in chunks
  int fh = filesystem_->open(file->inode());
  ASSERT_GE(fh, 0);
  ...
  while (total_read < fileSize) {
    size_t toRead = std::min(chunkSize, fileSize - total_read);
    size_t bytesRead = filesystem_->read(fh, readBuffer.data() + total_read,
                                         toRead, offset);
    EXPECT_EQ(bytesRead, toRead) << "Short read at offset " << offset;
    total_read += bytesRead;
    offset += bytesRead;
  }
  ...
  // Verify content is all zeros (spot check first/last/middle)
  EXPECT_EQ(readBuffer[0], 0);
  EXPECT_EQ(readBuffer[fileSize / 2], 0);
  EXPECT_EQ(readBuffer[fileSize - 1], 0);
}
