...
// NEW: Include domain converter and implementation header
...
// OUT-OF-LINE TEMPLATE MEMBER FUNCTION IMPLEMENTATIONS
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_devices(std::vector<uint64_t> devices) {
  md_.devices() = std::move(devices);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_symlink_table_size(size_t size) {
  md_.symlink_table()->resize(size);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_block_size(uint32_t block_size) {
  md_.block_size() = block_size;
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_shared_files_table(std::vector<uint32_t> shared_files) {
  md_.shared_files_table() = std::move(shared_files);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_category_names(std::vector<std::string> category_names) {
  md_.category_names() = std::move(category_names);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_block_categories(std::vector<uint32_t> block_categories) {
  md_.block_categories() = std::move(block_categories);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_category_metadata_json(std::vector<std::string> metadata_json) {
  md_.category_metadata_json() = std::move(metadata_json);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::set_block_category_metadata(
    std::map<uint32_t, uint32_t> block_metadata) {
  md_.block_category_metadata() = std::move(block_metadata);
}

template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::add_symlink_table_entry(size_t index, uint32_t entry) {
  DWARFS_NOTHROW(md_.symlink_table()->at(index)) = entry;
}

template <typename LoggerPolicy>
uint32_t thrift_metadata_builder<LoggerPolicy>::get_time_resolution() const {
...
template <typename LoggerPolicy>
uint32_t thrift_metadata_builder<LoggerPolicy>::get_subsec_mult() const {
  uint32_t mult = 0;
  if (md_.options()) {
...
template <typename LoggerPolicy>
std::chrono::nanoseconds thrift_metadata_builder<LoggerPolicy>::get_chrono_time_resolution() const {
...
void thrift_metadata_builder<LoggerPolicy>::update_inodes() {
  ...
  if (options_.timestamp) {
    auto const ts = timeres_->convert_offset(*options_.timestamp);
    for (auto& inode : md_.inodes().value()) {
      if (options_.keep_all_times) {
        inode.atime_offset() = ts;
        inode.ctime_offset() = ts;
      }
      inode.mtime_offset() = ts;
    }
  }

  auto const subsec_mult = get_subsec_mult();

  if (subsec_mult > 0) {
    auto const nano_resolution = std::chrono::nanoseconds{subsec_mult};
    auto const res = get_chrono_time_resolution();

    for (auto& inode : md_.inodes().value()) {
      inode.mtime_offset() = timeres_->convert_offset(
          inode.mtime_offset().value());

      if (options_.keep_all_times) {
        inode.atime_offset() = timeres_->convert_offset(
            inode.atime_offset().value());
        inode.ctime_offset() = timeres_->convert_offset(
            inode.ctime_offset().value());
      }
    }
  }
}

// Helper: builds Thrift internally (original build() logic)
template <typename LoggerPolicy>
...
thrift_metadata_builder<LoggerPolicy>::build() {
...
void thrift_metadata_builder<LoggerPolicy>::gather_chunks(inode_manager const& im,
                                                    ...
                                                    size_t chunk_count) {
  md_.chunk_table() = std::vector<uint32_t>();
  md_.chunk_table()->resize(im.count() + 1);
  md_.chunks() = std::vector<thrift::metadata::chunk>();
  ...
  bm.map_logical_blocks(domain_chunks, hole_mapper);

  // Convert back to Thrift
  md_.chunks()->clear();
  for (auto const& domain_chunk : domain_chunks) {
    md_.chunks()->push_back(metadata::converters::to_thrift(domain_chunk));
  }
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::gather_entries(
    std::span<dir*> dirs, global_entry_data const& ge_data,
    uint32_t num_inodes) {
  md_.dir_entries() = std::vector<thrift::metadata::dir_entry>();
  md_.inodes() = std::vector<thrift::metadata::inode_data>();
  md_.inodes()->resize(num_inodes);
  md_.directories() = std::vector<thrift::metadata::directory>();
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::gather_global_entry_data(
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::remap_blocks(std::span<block_mapping const> mapping, size_t new_block_count) {
  auto tv = LOG_TIMED_VERBOSE;

  std::span<thrift::metadata::chunk> old_chunks = md_.chunks().value();
  std::span<uint32_t> old_chunk_table = md_.chunk_table().value();
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::remap_holes(
    chunks_t& new_chunks, size_t new_hole_index, size_t max_data_chunk_size) {
  LOG_DEBUG << "remapping holes (hole index: " << md_.hole_block_index().value()
            << " -> " << new_hole_index << ")";

  auto const old_block_size = old_block_size_;
  ...
  for (auto& c : new_chunks) {
    if (c.block().value() == kTmpHoleIx) {
      ...
      // Convert Thrift chunk to domain chunk for hole mapping
      metadata::domain::chunk domain_chunk;
      domain_chunk.set_block(c.block().value());
      domain_chunk.set_offset(c.offset().value());
      domain_chunk.set_size(c.size().value());

      hole_mapper.map_hole(domain_chunk, hole_size);

      // Convert back to Thrift chunk
      c.block() = domain_chunk.block();
      c.offset() = domain_chunk.offset();
      c.size() = domain_chunk.size();
    }
  }
    ...
    if (dev_offset > reg_offset) {
      for (auto& de : md_.dir_entries().value()) {
        ...
        if (reg_offset <= inode_num && inode_num < dev_offset) {
          ...
          inode.nlink_minus_one() = inode.nlink_minus_one().value() + 1;
        }
      }
...
void thrift_metadata_builder<LoggerPolicy>::update_totals_and_size_cache() {
  ...
  if (reg_offset < dev_offset) {
    ...
    if (!md_.reg_file_size_cache()) {
      md_.reg_file_size_cache() = thrift::metadata::inode_size_cache();
    }
  ...
  if (options_.enable_sparse_files) {
    if (md_.total_allocated_fs_size().has_value()) {
      auto const old_allocated = md_.total_allocated_fs_size().value();
      if (old_allocated != total_allocated_fs_size) {
  ...
  if (md_.total_hardlink_size().has_value()) {
    auto const old_hardlink = md_.total_hardlink_size().value();
    if (old_hardlink != total_hardlink_size) {
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::upgrade_from_pre_v2_2() {
  // === v2.2 metadata ===
  //
  ...
  auto& dir_entries = newmd.dir_entries().ensure();
  dir_entries.reserve(md_.inodes()->size());
  auto& shared_files = newmd.shared_files_table().ensure();
...
template <typename LoggerPolicy>
void thrift_metadata_builder<LoggerPolicy>::upgrade_metadata(
    thrift::metadata::fs_options const* orig_fs_options,
    filesystem_version const& orig_fs_version) {
  auto tv = LOG_TIMED_VERBOSE;
...
// Constructor implementations for domain model (non-template, out-of-line)
...
thrift_metadata_builder<LoggerPolicy>::thrift_metadata_builder(
    ...
    metadata_options const& options)
    : LOG_PROXY_INIT(lgr)
    , md_{metadata::converters::to_thrift(md)}
    , options_{options}
    , old_block_size_{md_.block_size().value()}
    , timeres_{std::make_unique<time_resolution_converter>(
          options.time_resolution,
          get_conversion_factors(
              orig_fs_options ?
              std::make_unique<thrift::metadata::fs_options>(
                  metadata::converters::to_thrift(*orig_fs_options)).get()
              : nullptr))} {
  if (orig_fs_options) {
    auto thrift_opts = metadata::converters::to_thrift(*orig_fs_options);
    if (auto const feat = md_.features()) {
      features_.set(*feat);
      bool const non_sparse_image = !features_.has(feature::sparsefiles);
      DWARFS_CHECK(
    ...
    upgrade_metadata(&thrift_opts, orig_fs_version);
  } else {
    if (auto const feat = md_.features()) {
      features_.set(*feat);
    }
    upgrade_metadata(nullptr, orig_fs_version);
  }
  update_inodes();
}
...
thrift_metadata_builder<LoggerPolicy>::thrift_metadata_builder(
    ...
    metadata_options const& options)
    : LOG_PROXY_INIT(lgr)
    , md_{metadata::converters::to_thrift(std::move(md))}
    , options_{options}
    , old_block_size_{md_.block_size().value()}
    , timeres_{std::make_unique<time_resolution_converter>(
          options.time_resolution,
...
// Explicit template instantiations for logger policies
template class thrift_metadata_builder<debug_logger_policy>;
template class thrift_metadata_builder<prod_logger_policy>;

// Explicit template instantiations for templated constructor
template thrift_metadata_builder<debug_logger_policy>::thrift_metadata_builder(
    logger&, thrift::metadata::metadata&&,
    thrift::metadata::fs_options const*,
    filesystem_version const&,
    metadata_options const&);

template thrift_metadata_builder<prod_logger_policy>::thrift_metadata_builder(
    logger&, thrift::metadata::metadata&&,
    thrift::metadata::fs_options const*,
    filesystem_version const&,
    metadata_options const&);
...
template thrift_metadata_builder<debug_logger_policy>::thrift_metadata_builder(
    logger&, thrift::metadata::metadata const&,
    thrift::metadata::fs_options const*,
    filesystem_version const&,
    metadata_options const&);
