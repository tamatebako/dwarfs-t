...
public:
  ...
  domain::string_table read_string_table() const {
    ...
    // Debug: show the layout details
    if (layout_) {
      for (auto [field_id, field_ptr] : layout_->fields) {
      }
    }
    ...
    if (buffer_field) {
      ...
      auto buffer_layout_in_schema = schema_.layouts.get(buffer_layout_id);
      if (buffer_layout_in_schema) {
      } else {
      }

      auto buffer_reader = field_reader(1);

      // Check if buffer is wrapped in optional
      if (buffer_reader.layout_ && !buffer_reader.layout_->fields.is_empty()) {
        bool is_present = buffer_reader.field_reader(1).read_bool();

        // IMPORTANT: Even if is_some=0, try to read the buffer anyway for packed_names format
        // The data might be stored differently when packed_names is enabled
        if (is_present) {
          table.buffer = buffer_reader.field_reader(2).read_string();
        } else {
          // Try reading anyway - might be packed_names encoding
          // When is_some=0, field 2 might still have distance/len data
          try {
            auto field2_reader = buffer_reader.field_reader(2);
            if (field2_reader.layout_) {
              // Show sub-fields of field 2 if it's a struct
              if (!field2_reader.layout_->fields.is_empty()) {
                for (auto [fid, fptr] : field2_reader.layout_->fields) {
                }
              }
            }
            // Try reading the distance field directly
            try {
              // Check if field 2 is a struct with sub-fields (outlined string)
              if (field2_reader.layout_ && !field2_reader.layout_->fields.is_empty()) {
                uint32_t distance = field2_reader.field_reader(1).read_u32();
                uint32_t len = field2_reader.field_reader(2).read_u32();
                // Save buffer information for fallback calculation
                // When buffer.is_some=0, we still have the distance and len information
                // The buffer is located at (storage_start_ + distance) and has length len
                // We'll use this later to calculate the offset relative to symtab
                if (len > 0) {
                  buffer_distance = distance;
                  buffer_size = len;
                  // The distance is relative to storage_start_, so the absolute offset is:
                  // buffer_abs_offset = storage_start_ + distance
                  // We need to track this for the fallback calculation
                  symtab_distance = storage_start_ + distance + len;
                }
                // Only read the buffer if is_present=1
                if (is_present) {
                  uint32_t start = storage_start_ + distance;
                  uint32_t end = start + len;
                  if (end <= data_.size() && start <= end) {
                ...
                // Field 2 is a scalar (bits=9) - this is the buffer SIZE, not an offset
                // Homebrew mkdwarfs format: when buffer.is_some=0, the scalar value is the size
                // and the buffer is located right before the symtab
                try {
      ...
      if (symtab_reader.layout_ && !symtab_reader.layout_->fields.is_empty()) {
        ...
        if (is_present) {
          ...
          try {
            ...
            if (symtab_field2_reader.layout_) {
            }
    ...
    // Debug: Check total data size

    // Debug: Check condition for buffer search
  ...
  domain::metadata deserialize_metadata() const {
    ...
    if (auto field24 = layout_->fields.get(24)) {
      try {
        // First check if the optional wrapper has is_some set
        auto field24_reader = field_reader(24);
        auto field24_layout = schema_.layouts.get(field24->layout_id);
        if (field24_layout) {
          // Show sub-fields
          for (auto [sub_field_id, sub_field_ptr] : field24_layout->fields) {
          }
        }
        ...
        if (is_present) {
          ...
          if (!meta.compact_names->index.empty()) {
            ...
            // Debug: Check symtab and buffer status
            ...
            if (meta.compact_names->symtab.has_value() && !meta.compact_names->buffer.empty()) {
              ...
              try {
                ...
                for (size_t i = 0; i < num_chunks; ++i) {
                  ...
                  // Show first 15 chunks
                  if (i < 15) {
                  }
                }


                // Debug: Show first 100 bytes of decompressed buffer
