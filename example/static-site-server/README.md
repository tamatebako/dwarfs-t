...
1. **Download sample data** (Project Gutenberg books with images):
   ```bash
   ./download_samples.py --download all
   ```
...
3. **Build DwarFS images and run**:
   ```bash
   ./download_samples.py --setup all
...
|--------|--------|-------------|
| `aesop` | Project Gutenberg #11339 | Aesop's Fables with illustrations (116 images) |
| `candide` | Project Gutenberg #19942 | Voltaire's Candide with illustrations (4 images) |

### Download Commands

```bash
./download_samples.py --list           # List samples with status
./download_samples.py --download aesop # Download specific sample
./download_samples.py --setup all      # Download + build images
./download_samples.py --force --setup all  # Force rebuild
```

### Prerequisites for Image Building

To build DwarFS images, `mkdwarfs` must be available:

- **From project build**: Set `DWARFS_BUILD_DIR` environment variable
- **From Homebrew** (macOS): `brew install dwarfs`
- **Specify path**: Use `--mkdwarfs /path/to/mkdwarfs`
...
**Problem**: Download fails with SSL certificate error

**Solution**: Update Python certificates (macOS):
```bash
...
**Solution**: The file may have been updated on Project Gutenberg. Try forcing a re-download:
```bash
./download_samples.py --download aesop --force
```

### Image Building Issues

**Problem**: `mkdwarfs not found`

**Solution**: Ensure mkdwarfs is available:
```bash
# Option 1: Use project build
export DWARFS_BUILD_DIR=/path/to/dwarfs/build

# Option 2: Homebrew (macOS)
brew install dwarfs

# Option 3: Specify path directly
./download_samples.py --mkdwarfs /path/to/mkdwarfs --setup all
```

**Problem**: `Library not loaded: libfuse-t.dylib` on macOS

**Solution**: macOS System Integrity Protection (SIP) restricts library loading. Run with:
```bash
DYLD_LIBRARY_PATH=/usr/local/lib ./download_samples.py --setup all
```
